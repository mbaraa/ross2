...
class ActionChecker {
    ...
    public static async checkContestant(fn: () => void): Promise<void> {
        if ((await ContestantRequests.getProfile()).user.user_type_base == UserType.Contestant) {
            this.checkUser(fn);
        } else {
    ...
    public static async checkOrganizer(fn: () => void): Promise<void> {
        if ((await OrganizerRequests.getProfile()).user.user_type_base == UserType.Organizer) {
            this.checkUser(fn);
        } else {
